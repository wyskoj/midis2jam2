...
class ResolutionSettings(private val settings: Settings) {
    private val _isUseDefaultResolution = MutableStateFlow(settings.getBoolean(Resolution.IS_USE_DEFAULT_RESOLUTION, true))
    val isUseDefaultResolution: StateFlow<Boolean>
        get() = _isUseDefaultResolution

    private val _resolutionX = MutableStateFlow(settings.getInt(Resolution.RESOLUTION_X, 1920))
    val resolutionX: StateFlow<Int>
        get() = _resolutionX

    private val _resolutionY = MutableStateFlow(settings.getInt(Resolution.RESOLUTION_Y, 1080))
