...
class StandController(private val context: Midis2jam2) {
    private val keyboardStand = with(context.root) {
        +context.modelD("PianoStand.obj", "RubberFoot.bmp").apply {
            loc = v3(-50, 32, -6)
            rot = v3(0, 45, 0)
        }
    }

    private val malletStand = with(context.root) {
        +context.modelD("XylophoneLegs.obj", "RubberFoot.bmp").apply {
    ...
    private fun setStandVisibility(stand: Spatial, klass: KClass<out Instrument>) {
        stand.cullHint = context.instruments.any { klass.isInstance(it) && it.isVisible }.ch
    }
}
