...
void main(){
    ...
    vec3 computedRgb = AmbientSum       * diffuseColor.rgb  +
    DiffuseSum.rgb   * diffuseColor.rgb  * vec3(light.x) +
    ...
    // Convert to HSV
    vec3 hsv = computedRgb;

    #ifdef HUE_SHIFT
    hsv = rgb2hsv(computedRgb);
    hsv.x = mod(hsv.x + m_HueShift, 1.0);
    #endif

    #ifdef HUE_SHIFT_MASK
    vec3 shifted_rgb =  mix(computedRgb, hsv2rgb(hsv), texture2D(m_HueShiftMask, texCoord).r);
    #else
    vec3 shifted_rgb = hsv2rgb(hsv);
    #endif

    shifted_rgb = shifted_rgb * m_AOIntensity;

    gl_FragColor.rgb = shifted_rgb;
    #endif
