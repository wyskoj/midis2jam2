...
class Whistles(context: Midis2jam2, events: List<MidiChannelSpecificEvent>) :
	...
	override fun moveForMultiChannel(delta: Float) {
		offsetNode.setLocalTranslation(0f, 22.5f + indexForMoving(delta) * 6.8f, 0f)
	...
	init {
		IntStream.range(0, 12).forEach { i: Int -> whistleNodes[i] = Node() }
		for (i in 0..11) {
			twelfths[i] = Whistle(i).apply {
				highestLevel.setLocalTranslation(-12f, 0f, 0f)
			}
			whistleNodes[i].attachChild(twelfths[i]!!.highestLevel)
			whistleNodes[i].localRotation = Quaternion().fromAngles(0f, rad(7.5 * i), 0f)
			whistleNodes[i].setLocalTranslation(0f, 0.1f * i, 0f)
