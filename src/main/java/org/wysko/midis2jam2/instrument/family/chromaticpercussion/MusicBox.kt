...
class MusicBox(context: Midis2jam2, eventList: List<MidiChannelSpecificEvent>) : DecayedInstrument(context, eventList) {
	...
	override fun tick(time: Double, delta: Float) {
		...
		while (hitsItr.hasNext()) {
			val (noteTime, _, noteValue) = hitsItr.next()
		...
		while (pointsItr.hasNext()) {
			...
			val rotation = pointRotations[activePoint]
...
//		val xAngle = (0.5 * Math.PI * delta * (6E7 / context.file.tempoAt(tick).number) / 60.0).toFloat()
		...
		points.forEach {
			...
			pointRotations[it] = pointRotations[it]!! + xAngle
		}
	...
	init {
		hitsForRecoil = eventList.filterIsInstance<MidiNoteOnEvent>() as MutableList<MidiNoteOnEvent>
		instrumentNode.run {
			attachChild(context.loadModel("MusicBoxCase.obj", "Wood.bmp"))
			attachChild(context.loadModel("MusicBoxTopBlade.obj", "ShinySilver.bmp", REFLECTIVE, 0.9f))
			attachChild(cylinder)
			setLocalTranslation(37f, 7f, -5f)
		}
		cylinder.attachChild(context.loadModel("MusicBoxSpindle.obj", "ShinySilver.bmp", REFLECTIVE, 0.9f))
