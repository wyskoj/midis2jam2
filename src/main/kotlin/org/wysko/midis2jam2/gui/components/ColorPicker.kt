...
fun ColorPicker(
    ...
    Card(
        ...
        Row(
            Modifier.padding(16.dp).fillMaxHeight(), horizontalArrangement = Arrangement.spacedBy(16.dp)
        ) {
...
private fun HuePicker(
    ...
    Surface(
        modifier = Modifier.height(size.dp).width(64.dp).onPointerEvent(
        ...
        Canvas(modifier = Modifier.fillMaxSize()) {
            for (i in 0..size) {
                drawLine(
                    Color.hsv(
                        i / size.toFloat() * 360f, 1f, 1f
                    ), Offset(0f, i.toFloat()), Offset(size.toFloat(), i.toFloat())
                )
            }
            drawLine(
                Color.White,
...
private fun SaturationValuePicker(
    ...
    var mouseDown by remember { mutableStateOf(false) }
    Surface(
        modifier = Modifier.width(size.dp).height(size.dp).onPointerEvent(
            PointerEventType.Move
        ) {
            if (!mouseDown) return@onPointerEvent
            val pos = it.changes.first().position
            ...
            for (i in 0..size) {
                drawLine(
                    Brush.horizontalGradient(
                        listOf(
                            Color.hsv(hue, 0f, 1f - i / size.toFloat()), Color.hsv(hue, 1f, 1f - i / size.toFloat())
                        )
                    ), Offset(0f, i.toFloat()), Offset(size.toFloat(), i.toFloat())
                )
            }
            drawArc(
                Color.White, 0f, 360f, false, style = Stroke(2f), topLeft = Offset(
                    (saturation * size) - circleSize / 2f, (1f - value) * size - circleSize / 2f
